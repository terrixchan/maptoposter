...
This repository now includes a WeChat-compatible integration path:

- `wx_service/app.py`: HTTP API wrapper around the existing Python poster generator
- `wx-mini-demo/`: minimal WeChat Mini Program demo that downloads generated posters
...
- `GET /health`
- `GET /api/themes`
- `GET /api/posters/generate?city=Tokyo&country=Japan&theme=terracotta&fmt=png`

### 2. Configure Mini Program

Open `wx-mini-demo/` in WeChat DevTools and update:

- `wx-mini-demo/app.js`: set `YOUR_CLOUD_ENV_ID`
- `wx-mini-demo/pages/index/index.js`: set `CLOUD_ENV_ID` and `CLOUD_SERVICE`

`CLOUD_SERVICE` must match your WeChat Cloud Hosting service name and the backend should expose `/api/posters/generate-base64`.

### 3. Generate in Mini Program

The demo page will:

1. Call `/api/posters/generate-base64` via `wx.cloud.callContainer`
2. Display base64 image directly
3. Save poster to photo album (`wx.saveImageToPhotosAlbum`)

### Notes

- Poster generation still runs on server-side Python (OSMnx + Matplotlib).
- Mini Program only handles request/display/save.
- If you need Tencent Cloud deployment, you can deploy `wx_service` behind HTTPS and use the same demo frontend.
